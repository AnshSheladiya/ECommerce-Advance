...
exports.createUnwindStage = (field, includeArrayIndex = null) => {
  ...
  exports.createUnwindWithLookupStage = (field, lookupCollection, lookupFields) => {
    return { $unwind: { path: field, preserveNullAndEmptyArrays: true, lookup: { from: lookupCollection, localField: field, foreignField: lookupFields, as: field } } };
  };
  ...
  exports.createUnwindBySizeStage = (field, size, includeArrayIndex = null) => {
    if (includeArrayIndex === null) {
      return { $unwind: { path: field, preserveNullAndEmptyArrays: true, includeArrayIndex: 0, unwind: { path: '$' + field, filter: { $size: size } } } };
    } else {
