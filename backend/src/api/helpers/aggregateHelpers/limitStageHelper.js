...
exports.createLimitStage = (limit) => {
  ...
  exports.createPaginationWithSortAndMatchStage = (pageNumber, pageSize, sortField, sortOrder, matchField, matchValue) => {
  const skip = (pageNumber - 1) * pageSize;
  const limit = pageSize;
  return [{ $sort: { [sortField]: sortOrder } }, { $match: { [matchField]: matchValue } }, { $skip: skip }, { $limit: limit }];
  };
